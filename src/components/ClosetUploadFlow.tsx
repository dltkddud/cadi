...
export function ClosetUploadFlow({ onBack, onSaved }: { onBack: () => void; onSaved: (item: ClosetItem) => void }) {
  ...
  const handleFile = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = () => {
      // 원본은 최대 10MB라 전송/저장 전에 줄인다.
      void downscaleImage(String(reader.result)).then((resized) => {
  ...
  const startAnalysis = async () => {
    ...
    try {
      ...
      // analyzeClosetImage는 자체적으로 mock 폴백을 하지만,
      // 예상 못 한 예외로 "분석 중" 화면에 갇히지 않도록 한 번 더 막는다.
      console.error("[Cadi] 의류 분석 실패:", err);
      setErrorReason("의류 정보를 분석하지 못했습니다. 잠시 후 다시 시도해주세요.");
      setScreen("error");
    }
  };
...
// value는 라벨이 아니라 실제 값을 받는다. 예전에는 한국어 라벨을 받아
// `options.find(o => o.label === value)?.value`로 되돌렸는데, 매칭에 실패하면
// undefined가 들어가 select가 uncontrolled로 바뀌고 첫 옵션이 표시됐다.
